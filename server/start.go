...
`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			keyringBackend := clientCtx.Keyring.Backend()
			...
			serverCtx.Viper.Set(flags.FlagKeyringBackend, keyringBackend)
			...
			serverCtx.Logger.Debug(fmt.Sprint("received quit signal: ", err))
			return nil
		},
	}
...
func startInProcess(ctx *server.Context, clientCtx client.Context, opts StartOptions) (err error) {
	...
	grpcSrv, clientCtx, err := startGrpcServer(gctx, ctx, clientCtx, g, config.GRPC, app)
	if err != nil {
		return err
	}
	if grpcSrv != nil {
	...
	apiSrv := startAPIServer(gctx, ctx, clientCtx, g, config.Config, app, grpcSrv, metrics)
	if apiSrv != nil {
		defer apiSrv.Close()
	}

	clientCtx, httpSrv, httpSrvDone, err := startJSONRPCServer(ctx, clientCtx, g, config, genDocProvider, cfg.RPC.ListenAddress, idxer)
	if httpSrv != nil {
		defer func() {
	...
	if err := startRosettaServer(ctx, clientCtx, g, config); err != nil {
		return err
	}
...
func GenDocProvider(cfg *cmtcfg.Config) func() (*tmtypes.GenesisDoc, error) {
