	...
	testCases := []struct {
	...
	}{
		{
			...
			func() *types.MsgTransfer {
				...
				transferMsg := types.NewMsgTransfer("transfer", "channel-0", sdk.NewCoin("aphoton", sdkmath.NewInt(10)), senderAcc.String(), "", timeoutHeight, 0, "")

				coins := sdk.NewCoins(sdk.NewCoin("aphoton", sdkmath.NewInt(10)))
		...
		},
		{
			...
			func() *types.MsgTransfer {
				...
				transferMsg := types.NewMsgTransfer("transfer", "channel-0", sdk.NewCoin("erc20/"+contractAddr.String(), sdkmath.NewInt(10)), addr, "", timeoutHeight, 0, "")
				return transferMsg
			},
			false,
		},
		{
			...
			func() *types.MsgTransfer {
				...
				coin := sdk.NewCoin(pair.Denom, sdkmath.NewInt(10))
				...
				transferMsg := types.NewMsgTransfer("transfer", "channel-0", sdk.NewCoin(pair.Denom, sdkmath.NewInt(10)), senderAcc.String(), "", timeoutHeight, 0, "")
		...
		},
		{
			...
			func() *types.MsgTransfer {
				...
				coin := sdk.NewCoin("test", sdkmath.NewInt(10))
				coins := sdk.NewCoins(coin)
		...
		},
		{
			...
			func() *types.MsgTransfer {
				...
				coins := sdk.NewCoins(sdk.NewCoin(pair.Denom, sdkmath.NewInt(10)))
				err = suite.app.BankKeeper.MintCoins(suite.ctx, erc20types.ModuleName, coins)
				suite.Require().NoError(err)
				err = suite.app.BankKeeper.SendCoinsFromModuleToAccount(suite.ctx, erc20types.ModuleName, senderAcc, coins)
				suite.Require().NoError(err)
