	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			if tc.isNoOp {
				...
				suite.Require().Equal(new(uint256.Int).Add(prev, tc.amount).ToBig().Int64(), post.ToBig().Int64())
			}
		})
	}
}
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			if tc.isNoOp {
				...
				suite.Require().Equal(new(uint256.Int).Sub(prev, tc.amount).ToBig().Int64(), post.ToBig().Int64())
			}
		})
	}
}
...
func (suite *KeeperTestSuite) TestExist() {
	testCases := []struct {
		...
		{"success, has self destruct", suite.address, func(vmdb vm.StateDB) {
...
func (suite *KeeperTestSuite) TestPrepareAccessList() {
	...
	vmdb := suite.StateDB()
	vmdb.Prepare(params.TestRules, suite.address, coinbase, &dest, precompiles, accesses)
