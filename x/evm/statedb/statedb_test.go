...
func (suite *StateDBTestSuite) TestAccount() {
	...
	testCases := []struct {
		...
		{"self-destructed", func(db *statedb.StateDB) {
			// non-exist account.
...
func (suite *StateDBTestSuite) TestRevertSnapshot() {
	...
	testCases := []struct {
		...
		{"self destructed", func(db vm.StateDB) {
...
func (suite *StateDBTestSuite) TestAccessList() {
	...
	testCases := []struct {
		...
		{"prepare access list", func(db vm.StateDB) {
			...
			db.Prepare(params.TestRules, address, address3, &address2, vm.PrecompiledAddressesBerlin, al)
