...
var _ = Describe("ERC20:", Ordered, func() {
	...
	BeforeEach(func() {
		...
		tallyParams, _ := s.app.GovKeeper.Params.Get(s.ctx)
		tallyParams.Quorum = "0.0000000001"
		s.app.GovKeeper.Params.Set(s.ctx, tallyParams)
	})
	...
	Describe("Submitting a token pair proposal through governance", func() {
		Context("with existing coins", func() {
			BeforeEach(func() {
				...
				coins := sdk.NewCoins(
					sdk.NewCoin(testutil.BaseDenom, fundsAmt),
			...
			Describe("for a single Cosmos Coin", func() {
				BeforeEach(func() {
					...
					_, err = testutil.Delegate(s.ctx, s.app, privKey, sdk.NewCoin(testutil.BaseDenom, sdkmath.NewInt(500000000000000000)), s.validator)
					s.Require().NoError(err)
		...
		Context("with deployed contracts", func() {
			...
			Describe("for a single ERC20 token", func() {
				BeforeEach(func() {
					...
					//s.CommitAndBeginBlockAfter(duration)
					s.CommitAndBeginBlockAfter(time.Second * 172801)
					s.app.EndBlocker(s.ctx)
					s.Commit()
				})
	...
	Describe("Converting", func() {
		Context("with a registered coin", func() {
			BeforeEach(func() {
				...
				err := testutil.FundAccount(s.app.BankKeeper, s.ctx, accAddr, sdk.NewCoins(sdk.NewCoin(evmtypes.DefaultEVMDenom, fundsAmt)))
				s.Require().NoError(err)
...
func convertERC20(ctx sdk.Context, appEthermint *app.EthermintApp, pk *ethsecp256k1.PrivKey, amt sdkmath.Int, contract common.Address) {
	addrBz := pk.PubKey().Address().Bytes()
