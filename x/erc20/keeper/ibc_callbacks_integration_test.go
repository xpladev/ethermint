...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	evmosMeta := banktypes.Metadata{
		...
		DenomUnits: []*banktypes.DenomUnit{
			{
				Denom:    ibc.AevmosDenomtrace.BaseDenom,
				Exponent: 0,
			},
		},
		Name:    testutil.BaseDenom,
		Symbol:  erc20Symbol,
		Display: ibc.AevmosDenomtrace.BaseDenom,
	}
	...
	Describe("disabled params", func() {
		...
		It("should transfer and not convert to erc20", func() {
			...
			ibcOsmoBalanceBefore := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, ibc.UosmoIbcdenom)
			...
			ibcOsmoBalanceAfter := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, ibc.UosmoIbcdenom)
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert unregistered coin (uatom)", func() {
			...
			// check balance before transfer is 0
			ibcAtomBalanceBefore := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, ibc.UatomIbcdenom)
			...
			// check balance after transfer
			ibcAtomBalanceAfter := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, ibc.UatomIbcdenom)
		...
		It("should transfer and not convert aevmos", func() {
			...
			aevmosInitialBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, testutil.BaseDenom)

			// 1. Send aevmos from Evmos to Osmosis
			s.SendAndReceiveMessage(s.pathOsmosisEvmos, s.EvmosChain, testutil.BaseDenom, amount, receiver, sender, 1, "")

			aevmosAfterBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, testutil.BaseDenom)
			...
			aevmosIBCBalanceBefore := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, ibc.AevmosIbcdenom)
			s.Require().Equal(amount, aevmosIBCBalanceBefore.Amount.Int64())

			// 2. Send aevmos IBC coins from Osmosis to Evmos
			ibcCoinMeta := fmt.Sprintf("%s/%s", ibc.AevmosDenomtrace.Path, ibc.AevmosDenomtrace.BaseDenom)
			s.SendBackCoins(s.pathOsmosisEvmos, s.IBCOsmosisChain, ibc.AevmosIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)

			// check ibc aevmos coins balance on Osmosis - should be zero
			aevmosIBCSenderFinalBalance := s.IBCOsmosisChain.GetSimApp().BankKeeper.GetBalance(s.IBCOsmosisChain.GetContext(), senderAcc, ibc.AevmosIbcdenom)
			...
			aevmosFinalBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, testutil.BaseDenom)
			...
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, ibc.AevmosIbcdenom)
		...
		It("should transfer and convert original erc20", func() {
			...
			// check IBC Coin balance - should be zero
			ibcCoinsBalance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, ibc.UosmoIbcdenom)
			s.Require().Equal(int64(0), ibcCoinsBalance.Amount.Int64())
...
var _ = Describe("Convert outgoing ERC20 to IBC", Ordered, func() {
	...
	Describe("registered coin", func() {
		...
		It("should convert erc20 to ibc vouched and transfer", func() {
			...
			balance := s.app.BankKeeper.GetBalance(s.EvmosChain.GetContext(), senderAcc, ibc.UosmoIbcdenom)
			...
			s.SendBackCoins(s.pathOsmosisEvmos, s.EvmosChain, pair.Denom, amount, sender, receiver, 1, ibc.UosmoDenomtrace.GetFullDenomPath())
			s.IBCOsmosisChain.Coordinator.CommitBlock()
			// Check balance on the Osmosis chain
